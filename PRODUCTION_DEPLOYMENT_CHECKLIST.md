...
- [ ] **Environment Variables** (`apps/web/vercel.json` and/or Vercel Dashboard)
  ```bash
  # API Configuration
...
- [ ] **Scheduled Tasks Configured**
  - [ ] Hobby-safe scheduler enabled in `.github/workflows/keep_alive.yml` (`*/14 * * * *`)
  - [ ] If using Vercel cron on supported plans, configure in `apps/web/vercel.json`
...
**Solution**:
- On Hobby, verify `.github/workflows/keep_alive.yml` is enabled
- If using Vercel cron, verify cron paths in `apps/web/vercel.json`
- Check Vercel dashboard for cron execution logs (when Vercel cron is used)
- Ensure routes are exported as `GET` handlers and auth is configured
