...
| Loading↔results container parity (vΩ.25, 2026-07-27) | Frontend/motion tasks touching `match-loading-experience.tsx`: this layout has regressed three times. The interstitial container, its SSR skeleton, and the `match-selector.tsx` overlay wrapper must all stay at `max-w-6xl` to match `app/match/[id]/page.tsx`. Pinned by `match-loading-experience.test.tsx` — if that test fails, the fix is to restore parity, not to relax the assertion. |
