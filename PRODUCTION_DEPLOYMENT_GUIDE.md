...
### 4.1 Verify apps/web/vercel.json

Use `apps/web/vercel.json` as the canonical deployment config.

For Vercel Hobby, keep frequent schedules in GitHub Actions (`.github/workflows/keep_alive.yml`):
...
- **Hobby-safe frequent warmup:** Every 14 minutes via GitHub Actions (`*/14 * * * *`)
- **Vercel cron:** Use only for supported plan/schedule combinations; if enabled, define in `apps/web/vercel.json`
...
- Frequent backend warmup via GitHub Actions (`.github/workflows/keep_alive.yml`)
- Optional Vercel cron (plan-dependent) configured in `apps/web/vercel.json`
...
- **Cron Note:** Hobby does not support sub-daily cron in `apps/web/vercel.json`
...
2. Prefer GitHub Actions scheduler for frequent runs on Hobby plans
...
1. Verify `apps/web/vercel.json` is present and valid
2. For Hobby, verify `.github/workflows/keep_alive.yml` is enabled
3. Redeploy after configuration updates
4. Check Vercel project settings → Cron Jobs (if using Vercel cron)
