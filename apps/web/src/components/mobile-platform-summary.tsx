...
export function MobilePlatformSummary() {
  ...
  return (
    <div
      ...
      aria-label={
        health
          ? `Core ${core?.ready} of ${core?.total}; providers ${health.enabled} of ${health.configured}; models ${health.modelsReady ? "ready" : "unavailable"}`
    ...
    >
      <div className="flex min-h-10 items-center gap-1.5 rounded-md border border-white/10 bg-white/[0.03] px-2">
        ...
        <span className="min-w-0">
          <span className="block text-[9px] uppercase tracking-wide text-slate-500">Core</span>
      ...
      <div className="flex min-h-10 items-center gap-1.5 rounded-md border border-white/10 bg-white/[0.03] px-2">
        ...
        <span className="min-w-0">
          <span className="block text-[9px] uppercase tracking-wide text-slate-500">Providers</span>
          <span className="block truncate text-[11px] text-slate-200">
            {health ? `${health.enabled}/${health.configured}` : "…"}
          </span>
        </span>
      </div>
      <div className="flex min-h-10 items-center gap-1.5 rounded-md border border-white/10 bg-white/[0.03] px-2">
        ...
        <span className="min-w-0">
          <span className="block text-[9px] uppercase tracking-wide text-slate-500">Models</span>
          <span className="block truncate text-[11px] text-slate-200">
