/**
 * Phase 8 Sprint 2: Phase 8 Feature Intelligence Proxy
 *
 * GET /api/phase8-features/[matchId]
 *
 * Proxies requests to:
 * GET /api/v1/matches/upcoming/{matchId}/phase8-features
 *
 * Returns Phase8FeaturesResponse JSON.
 * When USE_PHASE8_FEATURES is not enabled the backend returns
 * status="disabled"; the frontend renders a disabled-state notice.
 *
...
export async function GET(
  ...
  try {
    ...
    console.error("[phase8-features proxy] fetch error:", message);
  ...
  if (isHtmlBody(body)) {
    console.error(
      "[phase8-features proxy] HTML response from backend (status %d) — cold start",
      backendRes.status,
    );
    return NextResponse.json(COLD_START_BODY, {
      status: 503,
  ...
  if (!backendRes.ok) {
    ...
    console.error(
      "[phase8-features proxy] backend error %d: %s",
      backendRes.status,
      body.slice(0, 200),
    );
    return NextResponse.json(
  ...
  try {
    ...
    console.error("[phase8-features proxy] non-JSON 200 body from backend");
    return NextResponse.json(
