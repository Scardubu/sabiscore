...
| Render provider ENABLE flags | Render dashboard (not code) | Live provider health re-confirmed 2026-08-07: `api_football`/`sportmonks`/`the_odds_api` still `enabled:false` (`provider_disabled`) despite `render.yaml` declaring `ENABLE_*=true` since vΩ.12; Blueprint env sync still awaits operator approval — unchanged across at least 5 sessions now. The three provider secrets are already reported configured and must never be copied into code or reports. |
