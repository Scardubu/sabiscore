...
| make verify (full 14-step) | `Makefile` | Gates 1–10, 13, 14 verified locally 2026-07-15. Gate 11 (backend image): `PIP_DEFAULT_TIMEOUT=120 PIP_RETRIES=5` in backend Dockerfile; re-run on fast network to close. Gate 12 (web image): Dockerfile correct as of vΩ.16 (node:22-slim, .dockerignore, 1 h retry timeout, gnu SWC); passes in CI. Locally, `SKIP_DOCKER_GATES=1 make verify` skips both to allow full pipeline on constrained networks. |
