...
| ⚠️ Production alias does NOT auto-promote (vΩ.21, 2026-07-24; re-verified vΩ.28) | Vercel dashboard (not code) | **Confirmed operator action — mechanism unchanged, but currently in sync.** Vercel builds every master commit (each gets a preview + the branch alias `web-git-master-oversabis-projects.vercel.app` auto-tracks HEAD), but the production alias `web-lac-theta-42.vercel.app` is pinned and does NOT auto-advance. Originally observed stuck at `cdb2a9d` while HEAD moved on; **vΩ.28 live-reconfirmed both aliases at `sha:97f3b38`, matching HEAD** — no promotion needed right now, but the underlying non-auto-promote behavior is unchanged and will drift again on a future push. Verify by comparing `/api/health` `sha` on the branch alias vs the production alias after every push. **To ship any pushed fix to production, promote the latest deployment: Vercel dashboard → Deployments → ⋯ → "Promote to Production", or `vercel promote <url>` / `vercel --prod`.** This promotion gap is exactly the deploy-parity drift the vΩ.19 `sha` stamp exists to expose. `sabiscore.com` does not resolve yet (re-confirmed vΩ.28, DNS lookup failure) — the custom domain is not attached to the `web` project. |
