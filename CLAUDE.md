...
| Off-season notice surfaced before submission on the match selector (vΩ.27, 2026-07-28) | A user could type any hypothetical matchup during the close season and only discover it was off-season after submitting, via the full-analysis "4 critical gaps / No bet" teardown (`FIXTURE_IDENTITY_UNVERIFIED` — correct, untouched). `match-selector.tsx` now runs a `useQuery(["match-selector-offseason", league], () => getUpcomingMatches({ league, days_ahead: 7, limit: 1 }))` — same 5 min/10 min stale/gc convention as the file's own `BigMatchesCarousel` — and renders the existing `LeagueOffseasonNotice` (previously used only in `upcoming-matches-panel.tsx`) above the Home/Away inputs when `offseasonData?.offseason` is true for the *currently selected* league. The display-form `league` state is passed unnormalized (no `canonicalLeagueId()`): `/api/upcoming` → `upcoming_matches.py` is a lenient boundary (lowercases + folds separators server-side), a different boundary from the strict Zod-enum proxies (`full-analysis`, `insights`, `phase8-features`) vΩ.26 hardened — this call exactly matches `upcoming-matches-panel.tsx`'s own already-production-verified usage of the same endpoint, so no normalization is needed or added. Renders nothing on loading, fetch error, in-season, or an unrecognized league — fails toward silence, never a false off-season claim. Non-blocking; `handleSubmit`, the backend, and `LeagueOffseasonNotice` itself are untouched. No new test file — neither `match-selector.tsx` nor `upcoming-matches-panel.tsx` (which has the identical conditional pattern) had one, and full-mounting this component requires mocking `next/navigation`, `react-hot-toast`, feature flags, and `next/dynamic`, disproportionate for one non-money conditional block. |
...
| ⚠️ Production alias does NOT auto-promote (vΩ.21, 2026-07-24) | Vercel dashboard (not code) | **Confirmed operator action.** Vercel builds every master commit (each gets a preview + the branch alias `web-git-master-oversabis-projects.vercel.app` auto-tracks HEAD), but the production alias `web-lac-theta-42.vercel.app` is pinned and does NOT auto-advance — observed stuck at `cdb2a9d` while HEAD moved to `25bafbe`→`b4f268f`. Verify the fix by comparing `/api/health` `sha` on the branch alias (current) vs the production alias (lagging). **To ship any pushed fix to production, promote the latest deployment: Vercel dashboard → Deployments → ⋯ → "Promote to Production", or `vercel promote <url>` / `vercel --prod`.** This promotion gap is exactly the deploy-parity drift the vΩ.19 `sha` stamp exists to expose. `sabiscore.com` does not resolve yet — the custom domain is not attached to the `web` project. |
| Render provider ENABLE flags | Render dashboard (not code) | Live provider health 2026-07-24 shows `api_football`/`sportmonks`/`the_odds_api` `enabled:false` despite render.yaml declaring `ENABLE_*=true` (vΩ.12) — blueprint env sync awaits operator approval in the Render dashboard, plus the 3 provider keys (`sync:false`). |
...
| make verify (full 14-step) | `Makefile` | Gates 1–3, 5–10, 13, 14 verified locally 2026-07-15. Gate 4 (Alembic): requires local Postgres — `backend/.env` Render URL is not resolvable off-Render. Pass `DATABASE_URL=postgresql://postgres:<pw>@localhost:5432/<db> make verify` to run it, or `SKIP_ALEMBIC_GATE=1 make verify` to skip (added 2026-07-16). Gate 11 (backend image): `PIP_DEFAULT_TIMEOUT=120 PIP_RETRIES=5` in backend Dockerfile; re-run on fast network to close. Gate 12 (web image): passes in CI. Skip both Docker gates locally: `SKIP_DOCKER_GATES=1 make verify`. Skip all infra-dependent gates: `SKIP_ALEMBIC_GATE=1 SKIP_DOCKER_GATES=1 make verify`. |
