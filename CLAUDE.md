...
| make verify (full 14-step) | `Makefile` | Gates 1–10, 13, 14 verified locally 2026-07-15 (gate 4 via local Postgres `sabiscore_verify`, see vΩ.15 rows). Remaining: gates 11–12 Docker image builds — network-bound on this connection (297 MB xgboost wheel); pip timeout hardening applied, re-run `docker build` for both images to close |
