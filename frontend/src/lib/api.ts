...
class APIClient {
  ...
  async getUpcomingMatches(daysAhead = 7, limit = 12, league?: string): Promise<UpcomingMatchesResponse> {
    const params = new URLSearchParams({
      days_ahead: String(daysAhead),
      limit: String(limit),
    })
    if (league) params.append('league', league)
    return this.request(`/matches/upcoming?${params.toString()}`)
  }
  ...
  async getPredictionUncertainty(matchId: string): Promise<{ match_id: string; uncertainty: UncertaintyBreakdown }> {
    return this.request(`/predictions/${encodeURIComponent(matchId)}/uncertainty`)
  }

  async getCausalFeatures(limit = 58): Promise<CausalFeaturesResponse> {
    return this.request(`/causal/features?limit=${Math.max(1, Math.min(limit, 200))}`)
  }
}
