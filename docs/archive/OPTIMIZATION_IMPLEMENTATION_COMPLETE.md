...
5. **`apps/web/vercel.json`**
  - Canonical deployment config for the `apps/web` Vercel project root
...
**Scheduler**: GitHub Actions keep-alive (`.github/workflows/keep_alive.yml`, every 14 minutes)
...
4. **Vercel Deployment**:
   ...
   vercel --prod
   ```
  - On Hobby, frequent scheduling runs via GitHub Actions
...
2. Monitor keep-alive workflow runs in GitHub Actions and cron route health in logs
