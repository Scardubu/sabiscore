...
try:
    _KELLY_CAP = get_league_policy("LIGUE_1").kelly_cap
except LeaguePolicyUnavailableError:
    _KELLY_CAP = 0.04  # ponytail: conservative fallback

class Ligue1Model:
    """
    Ligue 1-specific model accounting for:
    - Physicality
    - High variance due to young squads
    """
    ...
    def __init__(self, redis_client: redis.Redis):
        ...
        self.ensemble_weights = {
            'rf': 0.35, # Higher RF for variance handling
            'xgb': 0.35,
            'lgbm': 0.20,
            'gb': 0.10
        }
    ...
    def extract_ligue1_features(self, match_data: Dict) -> np.ndarray:
        """
        Extract Ligue 1-specific features
        """
        features = []
    ...
    def train(self, X_train: pd.DataFrame, y_train: pd.Series):
        ...
        for name, model in self.models.items():
            print(f"Training {name} for Ligue 1...")
        ...
        print("✅ Ligue 1 model training complete")
    ...
    def predict_proba(self, match_data: Dict) -> Dict[str, float]:
        ...
        X = self.extract_ligue1_features(match_data)
        ...
        for name, model in self.models.items():
            ...
            if len(pred) == 3:
                ...
                probs_draw += 0.26 * weight # Ligue 1 draw rate
