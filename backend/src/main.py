...
if cors_origins or cors_origin_regex:
    middleware_kwargs: dict = {
        "allow_origins": [str(origin).rstrip("/") for origin in cors_origins] if cors_origins else [],
    ...
    if cors_origin_regex:
        middleware_kwargs["allow_origin_regex"] = cors_origin_regex
    app.add_middleware(CORSMiddleware, **middleware_kwargs)
    logger.info(f"CORS enabled — exact origins: {cors_origins or []}, regex: {cors_origin_regex or 'none'}")
else:
