...
in `apps/web/vercel.json` to map `/api/v1/health` to the backend `/health` endpoint and pushed the change.

Summary:
- **Backend**: passed all checks consistently after attempt 2
- **Model loading**: passed (models loaded)
- **Frontend proxy `/api/v1/health`**: initially 502 — added `apps/web/vercel.json` rewrite to `/health` and pushed; re-check deployment logs if 502 persists
...
- [x] Add targeted `apps/web/vercel.json` rewrite for `/api/v1/health` (done)
