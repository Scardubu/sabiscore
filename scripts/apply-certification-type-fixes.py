"""Apply the reviewed production-runtime type contract and remove staging files."""
...
orchestrator = Path("backend/src/providers/orchestrator.py")
...
closure_replacements = {
    r"lambda e=espn, c=competition: e\.scoreboard\(c\)": (
        "lambda: espn.scoreboard(competition)"
    ),
    r"lambda f=fdo, c=competition: f\.fixtures\(competition=c\)": (
        "lambda: fdo.fixtures(competition=competition)"
    ),
    r"lambda f=fdo, c=competition: f\.standings\(competition=c\)": (
        "lambda: fdo.standings(competition=competition)"
    ),
    r"lambda a=apif, c=competition: a\.injuries\(competition=c\)": (
        "lambda: apif.injuries(competition=competition)"
    ),
    r"lambda a=apif, c=competition: a\.lineups\(fixture_id=fixture\.get\(\"provider_event_id\"\), competition=c\)": (
        'lambda: apif.lineups(fixture_id=fixture.get("provider_event_id"), competition=competition)'
    ),
    r"lambda a=apif, c=competition: a\.teams\(competition=c\)": (
        "lambda: apif.teams(competition=competition)"
    ),
    r"lambda s=sm, c=competition: s\.injuries\(competition=c\)": (
        "lambda: sm.injuries(competition=competition)"
    ),
    r"lambda a=apif, fid=fixture_id: a\.lineups\(fixture_id=fid\)": (
        "lambda: apif.lineups(fixture_id=fixture_id)"
    ),
    r"lambda s=sm, fid=fixture_id: s\.lineups\(fixture_id=fid\)": (
        "lambda: sm.lineups(fixture_id=fixture_id)"
    ),
    r"lambda o=odds_api, c=competition: o\.odds\(": "lambda: odds_api.odds(",
}
for pattern, replacement in closure_replacements.items():
    updated, matches = re.subn(pattern, replacement, text)
    if matches == 0:
        raise SystemExit(
            f"backend/src/providers/orchestrator.py pattern not found: {pattern}"
        )
    text = updated
...
if old_team_stats not in text:
    raise SystemExit(
        "backend/src/providers/orchestrator.py team statistics closure not found"
    )
